...
assessmentButton.addEventListener(
  'click',
   () => {//アロー関数
    ...
    //診断結果表示エリアの作成
    resultDivision.innerText = '';//divタグを空文字列で上書きする
...
function assessment(userName) {
  //全文字コードの番号を取得して足し合わせる
  let sumOfCharcord = 0;
  for (let i = 0; i < userName.length; i++) {
    sumOfCharcord = sumOfCharcord + userName.charCodeAt(i);
  }

  //文字コード番号の合計を回答の数で割って添字の数値を求める

  const index = sumOfCharcord % answers.length;
